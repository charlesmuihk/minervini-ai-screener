    ...
    for ticker, df in data.items():
        ...
        for idx in range(200, len(close)-HOLD_DAYS, SCAN_INTERVAL):
            ...
            if base_pass and vol >= VOL_MIN and regime == "BULL":
                tier = "A" if (is_vcp and dist>=-0.05) or (dist>=-0.05 and (rs or 0)>=90) else ("B" if dist>=-0.10 else "C")
                stop_c = entry * (1 - STOP_PCT_C)
                ret_v3 = ret_v1
                for d in range(1, exit_idx - idx):
                    if idx + d >= len(close): break
                    sp = stop_c if tier=="C" else stop_8
                    if close.iloc[idx+d] <= sp:
                        ret_v3 = -(STOP_PCT_C if tier=="C" else STOP_PCT_BULL)
                        break
            ...
            # v4: regime-aware sizing + tighter stop in bear
            if regime == "BULL":
                rs_min = MIN_RS
                ...
                pos_size = POS_BEAR
                tier_ok = True  # allow all tiers but tighter filter
            ...
            v4_pass = (
                ...
                dist >= -MAX_DIST and
                vol >= VOL_MIN and
                (regime != "BEAR" or is_vcp or (rs or 0) >= MIN_RS_BEAR)
            )
            ...
            if v4_pass:
                ...
                ret_v4 = ret_v4_raw * pos_size
                signals_v4.append({
                    "ticker": ticker, "date": date,
                    "ret": ret_v4, "ret_raw": ret_v4_raw,
                    "rs": rs, "trend": trend, "dist": dist,
                    "vol": vol, "is_vcp": is_vcp,
                    "tier": tier, "regime": regime,
                    "pos_size": pos_size, "stop_pct": stop_pct,
                })

    return signals_v1, signals_v2, signals_v3, signals_v4


def print_results(signals_v1, signals_v2, signals_v3, signals_v4, spy_rets):
    ...
    print("=" * 60)
    print("  V1 vs V2 vs V3 vs V4 BACKTEST COMPARISON (2020-2025, 3mo hold)")
    print("=" * 60)
    print()
    ...
    print(f"  {'Year':<6} {'V1 Avg':>8} {'V2 Avg':>8} {'V3 Avg':>8} {'V4 Avg':>8} {'V4 Win':>8}")
    print("  " + "-" * 58)
    for year in range(2020, 2026):
        ...
        w3,a3 = yr_stats(y3)
        w4,a4 = yr_stats(y4)
        print(f"  {year:<6} {a1:>8} {a2:>8} {a3:>8} {a4:>8} {w4:>8}")
    print("=" * 60)
...
if __name__ == "__main__":
    ...
    signals_v1, signals_v2, signals_v3, signals_v4 = run_backtest(data, spy)
    print(f"  DEBUG: v1={len(signals_v1)} v2={len(signals_v2)} v3={len(signals_v3)} v4={len(signals_v4)}")
    print_results(signals_v1, signals_v2, signals_v3, signals_v4, spy_rets)
