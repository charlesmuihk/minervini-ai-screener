...
REGIME_RULES = {
    "BULL":    {"max_pos": 4, "pos_pct": 0.15, "stop": 0.08, "min_rs": 70,  "vcp_only": False},
    "NEUTRAL": {"max_pos": 2, "pos_pct": 0.10, "stop": 0.06, "min_rs": 90,  "vcp_only": True},
    "BEAR":    {"max_pos": 1, "pos_pct": 0.05, "stop": 0.05, "min_rs": 90,  "vcp_only": True},
}

# Pyramiding rules: add at +10% and +20% from entry
PYRAMID_LEVELS = [
    {"trigger": 0.10, "add_pct": 0.10},
    {"trigger": 0.20, "add_pct": 0.10},
]
MAX_POS_PCT = 0.35  # max single position size
...
def run_portfolio_sim(data, spy_data):
    ...
    for scan_date in scan_dates:
        ...
        # Check pyramiding opportunities for existing positions
        for ticker, pos in list(portfolio.items()):
            ...
            gain = (cur_price / pos["entry"] - 1)

            for lvl in PYRAMID_LEVELS:
                lvl_key = f"pyramid_{lvl['trigger']}"
                if gain >= lvl["trigger"] and not pos.get(lvl_key, False):
                    cur_pct = pos["cost"] / INITIAL_CAP
                    if cur_pct + lvl["add_pct"] <= MAX_POS_PCT and capital > 0:
                        add_cash = INITIAL_CAP * lvl["add_pct"]
                        if add_cash > capital: add_cash = capital
                        add_shares = int(add_cash / cur_price)
                        if add_shares > 0:
        ...
        for ticker, pos in portfolio.items():
            if scan_date >= pos["exit_date"]:
                ...
                trades.append({"ticker":ticker,"entry_date":pos["entry_date"],
                    "exit_date":scan_date,"entry":pos["entry"],"exit":round(exit_price,2),
                    "ret":round(ret,2),"regime":pos["regime"],"is_vcp":pos["is_vcp"],
                    "tier":pos["tier"],"exit_reason":"time",
                    "pyramided":pos.get("pyramid_count",0)})
        ...
        for ticker, pos in portfolio.items():
            ...
            if cur_price <= pos["stop_price"]:
                ...
                capital += pos["cost"] + pnl
                ret = (cur_price / pos["entry"] - 1) * 100
                trades.append({"ticker":ticker,"entry_date":pos["entry_date"],
                    "exit_date":scan_date,"entry":pos["entry"],"exit":round(cur_price,2),
                    "ret":round(ret,2),"regime":pos["regime"],"is_vcp":pos["is_vcp"],
                    "tier":pos["tier"],"exit_reason":"stop",
                    "pyramided":pos.get("pyramid_count",0)})
        ...
        for ticker, df in data.items():
            ...
            candidates.append({"ticker":ticker,"score":sc,"rs":rs,"dist":dist,
                "is_vcp":is_vcp,"tier":tier,"idx":idx,"close":close})
        ...
        equity_curve.append({"date":scan_date,"equity":port_value,
            "regime":regime,"positions":len(portfolio)})
...
def print_portfolio_results(trades, equity_curve):
    ...
    print("=" * 60)
    print("  PORTFOLIO SIMULATION v3 RESULTS (2020-2025)")
    ...
    # Pyramiding stats
    pyramided = df[df["pyramided"] > 0]
    if len(pyramided):
        pw = pyramided[pyramided["ret"]>0]
        print(f"  Pyramided trades: {len(pyramided)} | Win {len(pw)/len(pyramided)*100:.0f}% | Avg {pyramided['ret'].mean():+.1f}%")
        non_pyr = df[df["pyramided"]==0]
        npw = non_pyr[non_pyr["ret"]>0]
        print(f"  Non-pyramided  : {len(non_pyr)} | Win {len(npw)/len(non_pyr)*100:.0f}% | Avg {non_pyr['ret'].mean():+.1f}%")
        print()
...
if __name__ == "__main__":
    print("=" * 60)
    print("  Portfolio Simulation v3 — Pyramiding + Tier A+VCP, 126-day hold")
    print("=" * 60)
