...
def run_backtest(data, spy_data):
    ...
    for ticker, df in data.items():
        ...
        for idx in range(200, len(close)-HOLD_DAYS, SCAN_INTERVAL):
            ...
            if base_pass and vol >= VOL_MIN and regime == "BULL":
                ...
                for d in range(1, exit_idx - idx):
                    if idx + d >= len(close): break
                    sp = stop_c if tier=="C" else stop_a
                    if close.iloc[idx+d] <= sp:
                        ret_v3 = -(STOP_PCT_C if tier=="C" else STOP_PCT_A)
                        break
    ...
    return signals_v1, signals_v2, signals_v3


def print_results(signals_v1, signals_v2, signals_v3, spy_rets):
    ...
    print("=" * 60)
    print("  V1 vs V2 vs V3 BACKTEST COMPARISON (2020-2025, 3mo hold)")
    print("=" * 60)
    print()
    ...
    print(f"  {'Year':<6} {'V1 Win':>8} {'V1 Avg':>8} {'V2 Win':>8} {'V2 Avg':>8} {'V3 Win':>8} {'V3 Avg':>8}")
    print("  " + "-" * 58)
    for year in range(2020, 2026):
        ...
        w2,a2 = yr_stats(y2)
        w3,a3 = yr_stats(y3)
        print(f"  {year:<6} {w1:>8} {a1:>8} {w2:>8} {a2:>8} {w3:>8} {a3:>8}")
    print("=" * 60)
...
if __name__ == "__main__":
    ...
    signals_v1, signals_v2, signals_v3 = run_backtest(data, spy)
    print(f"  DEBUG: v1={len(signals_v1)} v2={len(signals_v2)} v3={len(signals_v3)}")
    print_results(signals_v1, signals_v2, signals_v3, spy_rets)
